...
trap_init_percpu(void)
{
	...
	int n_cpu = cpunum();
	thiscpu->cpu_ts.ts_esp0 = KSTACKTOP - n_cpu * (KSTKSIZE + KSTKGAP);
	thiscpu->cpu_ts.ts_ss0 = GD_KD;
	thiscpu->cpu_ts.ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + n_cpu] = SEG16(STS_T32A, (uint32_t) (&(thiscpu->cpu_ts)),
					sizeof(struct Taskstate) - 1, 0);
	gdt[(GD_TSS0 >> 3) + n_cpu].sd_s = 0;
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if (tf->tf_cs == GD_KT) {
		panic("page fault with eip = %x", tf->tf_eip);
	}
	...
	if (curenv->env_pgfault_upcall) {
		...
		struct UTrapframe *frame = (struct UTrapframe *)(stacktop - size);
