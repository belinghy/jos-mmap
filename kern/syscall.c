...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	tf->tf_eflags = FL_IF;
	tf->tf_cs = GD_UT | 3;
	memcpy(&env->env_tf, tf, sizeof(env->env_tf));
	return 0;
}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	result = page_insert(env->env_pgdir, page, va, PTE_U | perm);
	if (result < 0) {
		page_free(page);
		return result;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	struct PageInfo *page = page_lookup(src_env->env_pgdir, srcva, 0);
	if (!page) {
		return -E_INVAL;
	}
	result = page_insert(dst_env->env_pgdir, page, dstva, PTE_U | perm);
	if (result < 0) {
		page_free(page);
		return result;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (perm) {
		...
		result = page_insert(env->env_pgdir, pp, env->env_ipc_dstva, PTE_U|perm);
		if (result < 0) {
			return result;
		}
	}
